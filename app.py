...
def get_session() -> requests.Session:
    ...
    now = time.time()
    cached = st.session_state.get("_nse_session")
    fetched_at = st.session_state.get("_nse_cookie_time", 0)

    if cached and (now - fetched_at) < COOKIE_TTL_SECONDS:
        return cached
    ...
    try:
        ...
        pass  # fetch calls below will surface the real error

    st.session_state["_nse_session"] = session
    st.session_state["_nse_cookie_time"] = now
    return session
...
def load_gainers():
    session = get_session()
    data = fetch_json(session, NSE_ENDPOINTS["gainers_losers"], REFERER_GAINERS)
...
def load_results():
    session = get_session()
    data = fetch_json(session, NSE_ENDPOINTS["corporate_results"], REFERER_RESULTS)
